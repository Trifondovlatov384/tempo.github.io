...
#### Если при «Обновить фид» ошибка SSL (tlsv1 alert internal error)

В коде используется **axios** с принудительным **TLS 1.2**, SNI и набором cipher suites (как рекомендовал программист).

Если ошибка остаётся:

1. Проверьте с сервера: `curl -v "ВАШ_FEED_URL_ИЗ_.env"`. Если curl успешен — TLS в Node ведёт себя иначе; если curl тоже падает — сетевая блокировка или ограничения Profitbase.
2. Запустите тестовый скрипт загрузки фида (отдельно от приложения):
   ```bash
   cd /home/app/tempo-nova
   npx tsx scripts/test-feed-fetch.ts
   ```
   В выводе будет видно, доходит ли запрос до ответа или падает с ошибкой.
3. Для **теста** без проверки сертификата (только чтобы убедиться, что дело в сертификате): в `.env` на VPS добавьте `FEED_INSECURE_SSL=1`, перезапустите приложение и снова нажмите «Обновить фид». Если после этого фид загружается — проблема в цепочке сертификатов или в том, как Profitbase отдаёт сертификат. В проде лучше не оставлять `FEED_INSECURE_SSL=1` без необходимости.
