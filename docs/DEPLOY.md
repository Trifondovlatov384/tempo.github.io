...
Ошибка возникает при запросе с VPS к HTTPS-фиду Profitbase. В коде используется **axios** с принудительным **TLS 1.2** и явным SNI — на большинстве VPS этого достаточно.

Если ошибка остаётся, проверьте с сервера, открывается ли фид:
```bash
curl -v "ВАШ_FEED_URL_ИЗ_.env"
```
Если `curl` тоже падает с SSL — возможна блокировка со стороны хостинга или Profitbase (по IP/региону). В таком случае стоит уточнить у хостинга или Profitbase.
